...
return [
    ...
    'name_fr' => ['required', 'string', 'max:255', Rule::unique('trainings')->whereNull('deleted_at')],
    'name_ar' => ['required', 'string', 'max:255', Rule::unique('trainings')->whereNull('deleted_at')],
        ...
        $validatedData = $this->validate();
         return DB::transaction(function () use ($validatedData, ) {
             ...
             $training= Training::create($validatedData);
               if ($this->image) {
                   $this->uploadAndCreateImage($this->image, $training->id, "App\Models\Training", "training");
               }
               return $this->response(true,message:__("forms.training.add.success-txt"));
